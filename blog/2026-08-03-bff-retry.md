...
Be honest about the cases where it's overkill:

- **One or two endpoints, single instance.** A small `p-retry` wrapper covers transient failures and asks nothing of a new dependency.
- **You need queueing, not rejecting.** `createRateLimiter` throws when the budget is spent. It doesn't hold the request and run it later. If your product wants "come back in 2 seconds and I'll serve you", reach for `execEvenly` to spread calls across the window, or a real queue.
- **The downstream is yours and never rate-limits.** If nothing upstream of you throttles, a client-side limiter has nothing to protect against.
- **You're already on React Query for the browser.** It doesn't run server-side, so it won't help here. It's the right tool for the frontend cache, though, because the two solve different layers. See the [react-query vs alova comparison](/about/comparison).

If your BFF calls a downstream that rate-limits, and you run more than one instance, the combination above is worth an afternoon.
