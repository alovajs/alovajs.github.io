...
`javascript // v2 accessAction('method-name', ({ send }) => { send(); }); // v3 accessAction( 'method-name', ({ send }) => { send(); }, true ); ` 18. `method.__key__` is simplified to method.key: `` javascript // v2 method.__key__ = 'custom-key'; // v3 method.key = 'custom-key '; ``` 19.  `method.transformData` is simplified to method.transform: ```javascript // v2 alova.Get('/api/profile', { transformData(data) { return data.detail; } }); // v3 alova.Get('/api/profile', { transform(data) { return data.detail;
}
});
