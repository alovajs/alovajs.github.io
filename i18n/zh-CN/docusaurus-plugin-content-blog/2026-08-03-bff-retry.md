...
- **一两个接口、单实例。** 一个小小的 `p-retry` 包裹就能 cover 瞬时失败，不用引入新依赖。
- **你要的是排队，不是拒绝。** `createRateLimiter` 额度用尽是抛错，不是把请求按住稍后跑。如果你的产品要的是「两秒后回来我给你服务」，用 `execEvenly` 把调用平摊到窗口里，或者上真正的队列。
- **下游是你自己的、从不限流。** 你上游没人掐你，客户端限流就没东西可护。
- **前端你已经在用 React Query。** 它不跑在服务端，所以帮不上这里的忙；但它管前端缓存是对的，两者管的是不同层。见 [react-query 与 alova 对比](/about/comparison)。
