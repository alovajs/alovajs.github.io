...
[这里有个`updateState`的 示例](/v2/tutorial/example/update-state)
...
可能有时候你并不确定需要更新 method 下的响应状态，但却知道以什么方式来找到需要失效的缓存数据，我们可以使用 [method 匹配器](/v2/tutorial/advanced/method-matcher) 来动态查找对应的 method 实例。以下例子展示了为名称为 todoList 的 method 实例对应的列表添加一条数据。
...
关于 [method 匹配器](/v2/tutorial/advanced/method-matcher) 将在后面的章节中详细介绍。
