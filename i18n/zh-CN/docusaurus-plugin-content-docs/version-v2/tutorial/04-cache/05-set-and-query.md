...
缓存也支持更新和查找，在[缓存模式](/v2/tutorial/cache/mode)中我们提到过，每份缓存数据是以发送请求的 method 实例作为 key 进行保存的，因此在手动更新缓存时也将使用 method 实例来查找对应的缓存数据。
...
同样的，你也可以通过 [method 匹配器](/v2/tutorial/advanced/method-matcher) 动态查找 method 实例。
...
你也可以通过 [method 匹配器](/v2/tutorial/advanced/method-matcher) 动态查找 method 实例。
